...
class AlgoliaObjectReference {
  ...
  String? get encodedObjectID => _objectId != null ? Uri.encodeFull(_objectId!) : null;
  ...
  Future<AlgoliaTask> setData(Map<String, dynamic> data) async {
    assert(_index != null && _index != '*' && _index != '', 'IndexName is required, but it has `*` multiple flag or `null`.');
  ...
  Future<AlgoliaTask> partialUpdateObject(Map<String, dynamic> data, {bool createIfNotExists = true}) async {
    assert(_objectId != null || createIfNotExists, 'You can\'t partialUpdateObject when createIfNotExists=false and data without an objectID.');
    assert(_index != null && _index != '*' && _index != '', 'IndexName is required, but it has `*` multiple flag or `null`.');
